"""MCP PostgreSQL server entrypoint."""

from mcp.server.fastmcp import FastMCP

from mcp_postgresql import tools

mcp = FastMCP(
    "mcp-postgresql",
    instructions=(
        "Read-only PostgreSQL MCP for production-safe exploration.\n"
        "\n"
        "Rules:\n"
        "- NEVER attempt writes (INSERT, UPDATE, DELETE, DROP, ALTER, TRUNCATE, "
        "CREATE, GRANT, COPY, etc.). They are blocked and the DB user is read-only.\n"
        ...
        "- Use Schema.Table when needed (default schema is often public).\n"
        "\n"
        ...
        "5) execute_query only for specific analytical SELECT/WITH/SHOW/EXPLAIN "
        "queries (row limits use LIMIT when needed).\n"
        "\n"
...
def test_connection() -> str:
    """Test the PostgreSQL connection and return database name and version."""
    return tools.test_connection()


@mcp.tool()
def list_tables() -> str:
    """List base tables in the configured PostgreSQL database."""
    return tools.list_tables()


@mcp.tool()
def list_views() -> str:
    """List views in the configured PostgreSQL database."""
...
def describe_table(table_name: str) -> str:
    """
    Describe the columns of a PostgreSQL table.

    Args:
        table_name: Table or Schema.Table (default schema public).
    """
...
def list_indexes(table_name: str) -> str:
    """
    List indexes for a PostgreSQL table.
...
def execute_query(query: str, limit: int = 100) -> str:
    """
    Execute a read-only SQL query (SELECT, WITH, SHOW, EXPLAIN).
