"""Install / print MCP client config for the PostgreSQL server."""
...
def postgresql_server_entry(root: Path, python: Path) -> dict[str, Any]:
    """Build the postgresql MCP server entry (no credentials)."""
    return {
        "command": str(python),
...
def build_snippet(root: Path, python: Path) -> dict[str, Any]:
    ...
    return {"mcpServers": {SERVER_NAME: postgresql_server_entry(root, python)}}
...
def merge_server(config: dict[str, Any], entry: dict[str, Any]) -> dict[str, Any]:
    """Insert/replace the postgresql server without touching other servers."""
    merged = dict(config)
...
def _collect_warnings(root: Path) -> list[str]:
    ...
    if not (root / ".env").exists():
        warnings.append(
            "Missing .env - copy .env.example to .env and set POSTGRES_* credentials."
        )
    else:
        ...
        if not has_pg:
            warnings.append(
                "No POSTGRES_* variables found in .env - add PostgreSQL credentials before connecting."
            )
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="python -m mcp_postgresql setup",
        description=(
            "Install the postgresql MCP server into a client config, "
...
def main(argv: list[str] | None = None) -> None:
    ...
    entry = postgresql_server_entry(root, python)
