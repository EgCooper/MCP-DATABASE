"""Informix connection helpers (pyodbc + Informix ODBC Driver)."""
...
from mcp_informix.config import InformixConfig


def get_connection(config: InformixConfig | None = None) -> pyodbc.Connection:
    """Open a new Informix connection via ODBC."""
    cfg = config or InformixConfig.from_env()
...
@contextmanager
def db_cursor(
    config: InformixConfig | None = None,
